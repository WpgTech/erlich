...
def deploy():
  with hide('everything'):            
    ...
    local("docker build -t erlich .")
    
    print("Tagging docker image")
    local("docker tag erlich wpgtech/erlich:latest")
    
    print("Pushing image to dockerhub")
    local("docker push wpgtech/erlich:latest")
    ...
    put(local_dir + "/docker-compose.yml", "/opt/lita/erlich/")

    print("Copying .env to remote server")
    put(local_dir + "/.env", "/opt/lita/erlich/")

    with cd('/opt/lita/erlich'):
      print("Pulling new docker image on remote server")
      run('/usr/local/bin/docker-compose pull')
    
    print("Restarting erlich on remote server")
    run('sudo systemctl restart erlich.service')